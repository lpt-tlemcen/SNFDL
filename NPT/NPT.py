...
read_input = open ("NPT_input_data.txt", "r") # Ouverture du fichier "NPT_input_data.txt" pour la lecture des
                                              # données de simulation

with open('NPT_input_data.txt') as f:
     for line in f:
          a = line.split()
          ...
          print("Tr =", Tr)
          Pr   = float(a[3])   # Préssion réduite
          print("Pr =", Pr)
          Eta  =  float(a[4])  # Paramètre du réservoir de chaleur
          print("Eta =", Eta)
          Peta =  float(a[5])  # Moment conjugué du paramètre Eta
          print("Peta =", Peta)
          Meta =  float(a[6])  # La Masse du réservoir de chaleur
          print("Meta =", Meta)
          Pv   =  float(a[7])  #Moment conjugué du paramètre de réservoir de pression
          print("Pv =", Pv)
          Mv =  float(a[8])  #La Masse du réservoir de pression
          print("Mv =", Mv)
          dt    = float(a[9])  # Delta(t) Pas de temps en unité réduite
          print("dt =", dt)
          Niter = int(a[10])    # Nombre d'itération
          print("Niter =", Niter)
          Niter_equilibrium = int(a[11]) # Nombre d'itération pour équilibrer le système
          print("Niter_equilibrium =", Niter_equilibrium)
          Niter_cor         = int(a[12]) # Nombre d'itération inclut dans le calcul du RMS
          print("Niter_cor =", Niter_cor)
          Conf_Type         = int(a[13]) # Nombre d'itération inclut dans le calcul du RMS
...
if (Conf_Type  == 1):
    ...
    fconf = open("Config_output_NPT_2.txt", "r")  # Ouverture du fichier "result_config_NPT _org" pour la lecture de la
    # configuration de sortie coordonnées et vitesses

    with open('Config_output_NPT_2.txt') as f:
        for line in f:
            a = line.split()
            indice = int(a[0])
            xpart = float(a[1])
...
f = open ("result_themod_NPT.txt", "w+") # Ouverture du fichier "result_themod_NPT.txt" pour l'écriture des données
                                         # pour visualisations
frms = open("result_RMS_NPT.txt", "w+")  # Ouverture du fichier "result_themod_NPT.txt" pour l'écriture des données
                                         # pour visualisations

fvaf = open("result_VAF_NPT.txt", "w+")  # Ouverture du fichier "result_VAF_NPT.txt" pour l'écriture des données
                                         #pour visualisations
...
# Fin du Calcul des Forces
    ...
    F_eta = Ec - 3 * (N+1) * Tr + Pv ** 2 / Mv

    Peta = Peta + dts2 * F_eta    #iL9 propagateur partiel

    Eta = Eta +  dts2 * Peta / Meta         #iL8 propagateur partiel

    Facteur_1 = math.exp( - dts2 * Peta / Meta )

    Pv = Pv * Facteur_1  #iL7 propagateur partiel

    F_v = (Ec + Virial)/(3*Volume) - Pr

    Pv = Pv + dts2 * F_v #iL6 propagateur partiel

    Facteur_2 = math.exp(- dts2 * (Peta / Meta + Pv / Mv / (3 * Volume)))

    for i in range(N):                 #iL5 propagateur partiel
        vx[i] = vx[i] * Facteur_2
        vy[i] = vy[i] * Facteur_2
        vz[i] = vz[i] * Facteur_2

    for i in range(N):                 #iL4 propagateur partiel
        vx[i] = vx[i] + dts2*Fx[i]
        vy[i] = vy[i] + dts2*Fy[i]
        vz[i] = vz[i] + dts2*Fz[i]
...
# On va s'assurer que la quantité totale de mouvement soit nulle
    ...
    Volume = Volume + dts2 * Pv /Mv             #iL3 propagateur partiel

    L = Volume**(1/3)

    Facteur_3 = math.exp( dts2 * Pv / Mv /(3*Volume) )
    ...
    for i in range(N):                 # iL2 propagateur partiel
                                       # iL1 propagateur central
                                       # iL2 propagateur partiel

        x_n[i] = (x[i]*Facteur_3 + dt*vx[i])* Facteur_3
        y_n[i] = (y[i]*Facteur_3 + dt*vy[i])* Facteur_3
        z_n[i] = (z[i]*Facteur_3 + dt*vz[i])* Facteur_3
    ...
    for i in range(N):
        ...
        if (x[i] > L) : print('fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!')
        ...
        if (y[i] > L) : print('fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!')
        ...
        if (z[i] > L) : print('fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!fuck!')

    # Les positions on changées dont on doit recalculer les Forces
...
fgr = open("result_Gr_NPT.txt", "w+")  # Ouverture du fichier "Gr_NPT.txt" pour l'écriture des données
                                       # pour visualisations
...
fconf = open("Config_output_NPT.txt", "w+")  # Ouverture du fichier "Config_output_NPT.txt" pour l'écriture de la
                                             # configuration de sortie coordonnées et vitesses
