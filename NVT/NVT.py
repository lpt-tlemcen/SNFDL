...
read_input = open ("NVT_input_data.txt", "r") # Ouverture du fichier "NVT_input_data.txt" pour la lecture des

with open('NVT_input_data.txt') as f:
     for line in f:
          a = line.split()
          ...
          print("Tr =", Tr)
          Eta  =  float(a[3])  # Paramètre du réservoir de chaleur
          print("Eta =", Eta)
          Peta =  float(a[4])  # Moment conjugué du paramètre Eta
          print("Peta =", Peta)
          Meta =  float(a[5])  # La Masse du réservoir de chaleur
          print("Meta =", Meta)
          Niter = int(a[6])    # Nombre d'itération
          print("Niter =", Niter)
          dt    = float(a[7])  # Delta(t) Pas de temps en unité réduite
          print("dt =", dt)
          Niter_equilibrium = int(a[8]) # Nombre d'itération pour équilibrer le système
          print("Niter_equilibrium =", Niter_equilibrium)
          Niter_cor         = int(a[9]) # Nombre d'itération inclut dans le calcul du RMS
          print("Niter_cor =", Niter_cor)
          Conf_Type = int(a[10]) # Nombre d'itération inclut dans le calcul du RMS
...
if (Conf_Type  == 1):
    ...
    fconf = open("Config_output_NVT.txt", "r")  # Ouverture du fichier "result_config_NVT _org" pour la lecture de la
    # configuration de sortie coordonnées et vitesses

    with open('Config_output_NVT.txt') as f:
        for line in f:
            a = line.split()
            indice = int(a[0])
            xpart = float(a[1])
...
f = open ("result_themod_NVT.txt", "w+") # Ouverture du fichier "result_themod_NVT.txt" pour l'écriture des données
                                         # pour visualisations
frms = open("result_RMS_NVT.txt", "w+")  # Ouverture du fichier "result_themod_NVT.txt" pour l'écriture des données
                                         # pour visualisations

fvaf = open("result_VAF_NVT.txt", "w+")  # Ouverture du fichier "result_VAF_NVT.txt" pour l'écriture des données
                                         #pour visualisations
...
for io in range(Niter):
    ...
    Peta = Peta + dts2 * (Ec - 3 * N * Tr)  # iL5 propagateur partiel

    Eta = Eta + dts2 * Peta / Meta  # iL4 propagateur partiel

    Facteur = math.exp(- dts2 * Peta / Meta)

    for i in range(N):  # iL3 propagateur partiel
        vx[i] = vx[i] * Facteur
        vy[i] = vy[i] * Facteur
        vz[i] = vz[i] * Facteur
    ...
    for i in range(N):  # iL1 propagateur partiel (central)
...
fgr = open("result_Gr_NVT.txt", "w+")  # Ouverture du fichier "Gr_NVT.txt" pour l'écriture des données
                                       # pour visualisations
...
fconf = open("Config_output_NVT.txt", "w+")  # Ouverture du fichier "Config_output_NVT.txt" pour l'écriture de la
                                             # configuration de sortie coordonnées et vitesses
