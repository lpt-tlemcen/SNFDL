...
Pr = 10                   # Pression réduite
ncfc=round((N/4.)**(1/3)) # Nombre de petites boîtes CFC
Vol = N/Ro                # Volume de la boîte
L = Vol**(1/3)            # Longueur réduite de la boîte
...
Eta  = 0                 #Paramètre du réservoir de chaleur
Peta = 0                 #Moment conjugué du paramètre Eta
Meta = 0.01              #La Masse du réservoir de chaleur

Pv = 0                 #Moment conjugué du paramètre de réservoir de pression
Mv = 0.005             #La Masse du réservoir de pression
...
f = open ("result_themod_NPT.txt", "w+") #Ouverture du fichier "result_themod_NVE.txt" pour l'écriture des données
                                         #pour visualisations
...
# Fin du Calcul des Forces
    ...
    Peta = Peta + dts2 *(Ec-3*N*Tr+Pv**2/Mv/(3*Vol))    #iL9 propagateur partiel

    Eta = Eta + dts2*Peta/Meta         #iL8 propagateur partiel

    Pv = Pv*math.exp(-dts2*Peta/Meta)  #iL7 propagateur partiel

    Pv = Pv + dts2*(Ro*(Ec + Virial)/(3*N) - Pr)/(3*Vol) #iL6 propagateur partiel

    Facteur = math.exp(- dts2*(Peta/Meta+Pv/Mv/(3*Vol)))
...
# On va s'assurer que la quantité totale de mouvement soit nulle
    ...
    Vol = Vol + dts2*Pv/Mv             #iL3 propagateur partiel

    L = Vol**(1/3)

    for i in range(N):                 # iL2 propagateur partiel (central)

        x[i] = x[i]*math.exp(dts2*Pv/Mv/(3*Vol))
        y[i] = y[i]*math.exp(dts2*Pv/Mv/(3*Vol))
        z[i] = z[i]*math.exp(dts2*Pv/Mv/(3*Vol))
    ...
    for i in range(N):                 #iL2 propagateur partiel (central)
        ...
        xn = x[i]*math.exp(dts2*Pv/Mv/(3*Vol))
        ...
        yn = y[i]*math.exp(dts2*Pv/Mv/(3*Vol))
        ...
        zn = z[i]*math.exp(dts2*Pv/Mv/(3*Vol))
